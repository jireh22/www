<?
   function latest_article($table, $loop, $char_limit) // 테이블명 , 리스트개수 , 제목글제한수(byte)
   {
	  ...
	  //latest_article("concert", 5, 30);
		...
		{
			$num = $row[num];
			$len_subject = mb_strlen($row[subject], 'utf-8'); //한글도 1자로 처리 , 제목의 총글자수 30
			$subject = $row[subject];
			...
			// if ($len_content > 50) //제한글자수(30) 보다 크면
			// {
			// 	//$subject = str_replace( "&#039;", "'", $subject);               
            ...
            if($table=='concert'){
                ...
                if($row[file_copied_0]){  //첨부된 이미지가 있으면..
                 $concertimg='./concert/data/'.$row[file_copied_0]; // '.concert/data/2022_02_22_10_43_06_0.jpg'
                }else{
