...
$(function() {                                   
    ...
    var fragment = $('#content #entertainment li a.current').attr('href');   //'data.html#Modeling'
       
    fragment = fragment.replace('#', ' #');            //'data.html #Modeling'    한칸띄우라는 말임  
    $('#details').load(fragment);    //$('선택자').load('파일명.html'); 
       
     $('#content #entertainment li a').on('click', function(e) { 
       ...
       $('#entertainment a.current').removeClass('current');    //커렌트있다면 커렌트 없에버리고 자기만 탭활성화 시켜라   
